        ...
        if (itemStack.getItem() == Items.SHEARS){
            if (!world.isClient){
                ...
                Direction direction2 = direction.getAxis() == Direction.Axis.Y ?
                        player.getHorizontalFacing().getOpposite() : direction;
                world.playSound(null, pos, SoundEvents.BLOCK_PUMPKIN_CARVE, SoundCategory.BLOCKS, 1.0F, 1.0F);
                world.setBlockState(pos, BlockRegistry.CARVED_WHITE_PUMPKIN_BLOCK.getDefaultState().with(CarvedWhitePumpkinBlock.FACING, direction2), 11);
                ItemEntity itemEntity = new ItemEntity(world, pos.getX() + 0.5D + direction2.getOffsetX() * 0.65D,
                        pos.getY() + 0.1D, pos.getZ() + 0.5D + direction2.getOffsetZ() * 0.65D,
